...
def select(cur, subspace, breakdown, aggregated_measure, table_name):
    # 获得当前breakdown的data scope
    sql = 'select ' + breakdown + ',' + aggregated_measure + ' from ' + table_name + ' where true '
...
def construct_HDS(subspace, breakdown, aggregation_list, measure_list, table_name):
    conn = get_conn()
    cur = conn.cursor()
    HDS = {}
    imp_dict = {}
    for measure in measure_list:
        for aggregation in aggregation_list:
            aggregated_measure = aggregation + '(' + measure + ')'
            res = select(cur, subspace, breakdown, aggregated_measure, table_name)
            imp = get_imp(cur, subspace, measure, table_name)
            for item in res:
                if aggregated_measure not in HDS.keys():
                    HDS[aggregated_measure] = {}
                if item[0] not in HDS[aggregated_measure].keys():
                    HDS[aggregated_measure][item[0]] = float(item[1])
            imp_dict[aggregated_measure] = imp
    conn.close()
    return HDS, imp_dict


def construct_HDP(HDS, insight_type, imp_dict):
    res = []
    for aggregated_measure in HDS.keys():
        flag, highlight, sig, values = dp(HDS[aggregated_measure], insight_type)
        if flag:
            imp = imp_dict[aggregated_measure]
            res.append((aggregated_measure, insight_type, highlight, imp, sig, imp*sig, values))
    return sorted(res, key=lambda x: x[5], reverse=True)


# 最大一项比例大于平均占比的五倍, highlight为占比最高的一项, sig = 占比最高的一项 / (占比最高的一项 + 各项占比的平均值)
...
def measure_extending(table_name, insight_type, subspace, breakdown, aggregation_list, measure_list, seed_measure, result_path):
    print('Measure extending for', table_name, insight_type)
    start_time = time.perf_counter()
    data_scope, imp_dict = construct_HDS(subspace, breakdown, aggregation_list, measure_list, table_name)
    commonness = construct_HDP(data_scope, insight_type, imp_dict)
    ...
    for item in commonness:
        if seed_measure == item[0]:
            satisfying_current_type = True
            csv_writer.writerow(['Seed Insight', subspace, breakdown, seed_measure, str(item[1]), str(item[2]), str(item[3]), str(item[4]), str(item[5]), str(item[6])])
            commonness.remove(item)
    if not satisfying_current_type:
        csv_writer.writerow(
            ['Seed Insight', subspace, breakdown, insight_type, seed_measure, None, None, None, None, None])
    for item in commonness:
        csv_writer.writerow(['Measure Extending', subspace, breakdown, str(item[0]), str(item[1]), str(item[2]), str(item[3]), str(item[4]), str(item[5]), str(item[6])])
    commonness_file.close()
    print('Measure extending down!\nTime:', str(time.perf_counter() - start_time) + 's')


if __name__ == '__main__':
    for T in insight_types:
        measure_extending(table_name='country',
                          insight_type=T,
                          subspace={'year': '*', 'region': '*', 'country': 'Iraq'},
                          breakdown='year',
                          aggregation_list=['sum', 'avg', 'count', 'max', 'min'],
                          measure_list=['count'],
                          seed_measure='sum(count)',
                          result_path='../result/commonness/measure_extending/')
    print('Done!!!')
